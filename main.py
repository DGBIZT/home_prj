...
def choose_difficulty():
        ...
        if third_question == yes:
            ...
            for item in sorted_in_ascending_order:
                if "operationAmount" in item:
                    has_rub = any(
                        item["operationAmount"]["currency"]["code"] == "RUB" for item in sorted_in_ascending_order
                    )
                    if not has_rub:
                    ...
                    if currency == "RUB":
                        ...
                        break
                if "operationAmount" not in item:
    ...
    if len(categories_list) != 0:
        ...
        for operation in certain_word_list_of_dicts:
            if "operationAmount" not in operation:
